...
### ⚡ 1. Go-Powered Concurrency (High-Throughput Bot Fleet)
...
### 🔍 3. Real-Time Correctness & Chaos Auditing
The load worker doesn't just calculate speed (TPS/latency); it acts as an **Active Correctness Auditor** during the stress test:
* **Crossed Book Check**: Periodically inspects orderbook spreads. If Bid $\ge$ Ask, a priority violation is logged.
...
### 📊 4. Cyberpunk Trading Cockpit HUD
...
├── templates/
│   └── go/                      # Reference Matching Engine template (Go)
├── sandbox-manager/             # FastAPI Container Sandboxing Daemon (Python)
├── load-generator/              # High-throughput Bot Fleet & Ingester (Go)
├── dashboard/                   # Express WebSocket Server & HUD Client (Node)
├── tests/                       # Automated Correctness & Chaos Test Suite
└── iac/                         # Infrastructure as Code (K8s & Terraform)
```
...
## 🛡️ Running Chaos & Correctness Tests
...
## ☁️ Production Cloud Deployment (IaC)
...
* **Production Sandbox Hardening**: In public settings, Docker socket mounting is replaced by AWS Fargate task creation (powered by AWS Firecracker microVMs) to prevent container escape exploits. Detailed configurations are explained in **[ARCHITECTURE_BLUEPRINT.md](ARCHITECTURE_BLUEPRINT.md)**.
